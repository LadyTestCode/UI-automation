...
def test_moving_menu_links_boxes():
    try:
        ...
        driver.get("http://www.sberbank.ru/")
        driver.maximize_window()
        boxes_rates_button = driver.find_element_by_xpath("(//a[text()=\"Банкоматы\"])[2]")
...
@pytest.mark.parametrize('element_menu, element_title, element_text',
                         ...
                         ids=["Exchange", "Offices", "Sbol", "Boxes"]
                         )
def test_moving_menu_links_parametrized(element_menu, element_title, element_text):
    try:
        ...
        driver.get("http://www.sberbank.ru/")
        driver.maximize_window()
        boxes_rates_button = driver.find_element_by_xpath(element_menu)
        boxes_rates_button.click()
        time.sleep(5)
...
def test_count_links():
    try:
        ...
        boxes_rates_count = driver.find_element_by_xpath("(//a[text()=\"Банкоматы\"])[2]")
...
def test_color_link():
    ...
    #проверим остальные ссылки: "Курсы валют", "Офисы", "Банкоматы".
        ...
        boxes_button = driver.find_element_by_xpath("(//a[text()=\"Банкоматы\"])[2]")
